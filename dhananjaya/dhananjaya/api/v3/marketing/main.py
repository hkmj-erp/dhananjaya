...
from dhananjaya.dhananjaya.utils import encode_donation_id, get_company_defaults, get_short_url
...
def upload_donation():
	...
	try:
		...
		preacher = frappe.db.get_single_value("Dhananjaya Settings", F_DEFAULT_MARKETING_PREACHER)
		if preacher is None:
	...
	long_url = "https://"+ frappe.local.site + f"/api/method/dhananjaya.dhananjaya.utils.download_pdf_public?receiptToken={receipt_token}"

	return frappe._dict(
		url=get_short_url(long_url)
	)

COMPULSORY_FIELDS = [F_MOBILE, F_DONOR_NAME, F_ADDRESS, F_COMPANY, F_DATE, F_PAYMENT_METHOD,F_AMOUNT, F_SEVA_TYPE]
