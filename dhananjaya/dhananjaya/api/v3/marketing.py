...
from dhananjaya.utils import encode_donation_id, get_company_defaults
...
def upload_donation(t):
    ...
    ## Preacher Identify
    try:
        t["preacher"] = t.get("preacher").strip()
        preacher = frappe.get_doc("LLP Preacher", t["preacher"]).name
    except:
        preacher = frappe.db.get_single_value("Dhananjaya Settings", "default_marketing_preacher")
        if preacher is None:
            frappe.throw("Please set the default Marketing Preacher in Dhananajaya Settings first.")

    donor = None

    clean_pan = None
    if t.get("pan_no"):
        clean_pan = re.sub(r"\s+", "", t.get("pan_no"))
    ...
    clean_contact = None
    if (donor is None) and t.get("mobile"):
    ...
    receipt_dict = {
        ...
        "receipt_date": t.get("date"),
        "preacher": llp_preacher,
        "donor": donor,
        "payment_method": t.get("payment_method"),
        "amount": t.get("amount"),
        "remarks": t.get("remarks"),
        "seva_type": t.get("seva_type"),
        "seva_subtype": t.get("seva_subtype"),
        "old_dr_no": t.get("dr_no"),
        "additional_charges": t.get("additional_charges"),
        "print_remarks_on_receipt": t.get("print_remarks_on_receipt"),
    }
    ...
    defaults = get_company_defaults(t.get("company"))

    if defaults.auto_create_journal_entries:
