	...
	refresh: function (frm) {
		...
		frm.add_custom_button(__('Bounce'), function () {
			...
			if (frm.doc.bounce_transaction) {
				warning_message = 'This action will consequently create a bounced Journal Entry and reconcile the Bank Statement as well.';
			} else {
				warning_message = 'This action will directly put the receipt in Bounced State without going through Realization.'
				if (frm.doc.docstatus == 1) {
					frappe.msgprint("Bounce Transaction is mandatory for Realized State")
					return;
				}
			}
