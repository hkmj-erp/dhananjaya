...
def import_preachers(*args,**kwargs):
	data = run_query("""select PREACHER_CODE
						from view_receipt_details vrd 
						union 
						select PREACHER_CODE  
						from view_donor_details""")
	for d in data:
		preacher = frappe.get_doc(doctype='LLP Preacher', initial=d['PREACHER_CODE'], full_name = d['PREACHER_CODE'])
...
def import_salutations(*args,**kwargs):
	...
	for d in data:
		if d['SALUTATION'] is not None and d['SALUTATION'].strip() != "":
			salutation = frappe.get_doc(doctype='Salutation', salutation=d['SALUTATION'])
...
def import_donors(*args,**kwargs):
	...
	data = run_query(f"SELECT * from `view_donor_details` WHERE TRUST_ID IN ({companies}) {test_string}")
	ind = 0
...
def set_patron_in_donors(*args,**kwargs):
	settings = frappe.get_cached_doc("Dhananjaya Import Settings")
	companies =	','.join([str(c.old_trust_code) for c in settings.companies_to_import])
	patrons = run_query(f"""SELECT PATRON_ID, group_concat(DONOR_ID) as donors, count(PATRON_ID) as count
						from (
...
def set_latest_preacher_in_donors(*args,**kwargs):
	donor_preachers = run_query(f"""select vrd.DONOR_ID ,vrd.PREACHER_CODE 
									from view_receipt_details vrd 
...
def export_defective_patrons(*args,**kwargs):
	...
	patrons = run_query(f"""SELECT vrd.PATRON_ID,CONCAT(vpd.FIRST_NAME," ", COALESCE(vpd.LAST_NAME,"")) as patron_name,
	...
	donor_ids = [ [p['PATRON_ID'], p['patron_name'] , p['donors'], p['count']] for p in patrons if p['count'] != 1]
	
	header_row = ["patron","patron_name","count"]+[f"donor{i}" for i in range(1, max_count)]
	rows = [header_row]
...
def upload_patron_correction_template(*args,**kwargs):
	...
	settings = frappe.get_cached_doc("Dhananjaya Import Settings")
	_file = frappe.get_doc("File", {"file_url": settings.patron_correction_template_file})
