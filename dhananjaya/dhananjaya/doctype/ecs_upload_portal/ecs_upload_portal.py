...
def get_data_in_dict():
	...
	headers = rows[0]
	# return headers
	if not ['Unique Reference Number','Transaction ID','Presentment Mode','Customer Name','Amount','Date','Status',	'Reason Code','Reason description'] == headers:
...
def process_ecs_data():
	...
	for d in gs_data:
		...
		if d['Unique Reference Number'] not in donor_data:
			frappe.throw(f" Unique Reference Number <b>{d['Unique Reference Number']}</b> is not available in Donor Data.")
		else:
			ecs_data[d['Unique Reference Number']].update({'donor_id':donor_data[d['Unique Reference Number']]['name']})
